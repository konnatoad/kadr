    ...
    fn show_general(&mut self, ui: &mut Ui) {
        ...
        ui.horizontal(|ui| {
            ...
            egui::ComboBox::from_id_salt("settings_monitor")
                ...
                .show_ui(ui, |ui| {
                    if ui
                        .selectable_label(self.preferred_monitor == 0, "Auto (OS default)")
                        .clicked()
                    {
                    ...
                    for m in &self.monitors {
                        let idx = m.index + 1;
                        if ui
                            .selectable_label(self.preferred_monitor == idx, m.label())
                            .clicked()
                        {
        ...
        ui.add_space(4.0);
        widgets::pill_checkbox(ui, &mut self.show_thumbnails, "Show thumbnail strip");
        ui.add_space(2.0);
        widgets::pill_checkbox(ui, &mut self.scan_subfolders, "Scan subfolders");

        ui.add_space(12.0);
        widgets::section_label(ui, "Filter");
        ui.add_space(4.0);
        widgets::pill_checkbox(ui, &mut self.filter_images, "Show images");
        ui.add_space(2.0);
        widgets::pill_checkbox(ui, &mut self.filter_videos, "Show videos");

        ui.add_space(12.0);
        widgets::section_label(ui, "Playback");
        ui.add_space(4.0);
        widgets::pill_checkbox(ui, &mut self.loop_videos, "Loop videos");
        ...
        ui.add_space(4.0);
        widgets::pill_checkbox(
            ui,
            &mut self.remember_last_folder,
    ...
    fn show_slideshow(&mut self, ui: &mut Ui) {
        ...
        ui.horizontal(|ui| {
            ...
            if !self.lua_code.trim().is_empty() {
                ui.add_space(4.0);
                if ui
                    .button(RichText::new("Clear").color(theme::TEXT_DIM).size(12.0))
                    .clicked()
                {
                    self.lua_code.clear();
                    self.lua_error = None;
...
fn key_to_serializable(key: Key) -> Option<SerializableKey> {
    Some(match key {
        ...
        Key::A => SerializableKey::A,
        Key::B => SerializableKey::B,
        Key::C => SerializableKey::C,
        Key::D => SerializableKey::D,
        Key::E => SerializableKey::E,
        Key::F => SerializableKey::F,
        Key::G => SerializableKey::G,
        Key::H => SerializableKey::H,
        Key::I => SerializableKey::I,
        Key::J => SerializableKey::J,
        Key::K => SerializableKey::K,
        Key::L => SerializableKey::L,
        Key::M => SerializableKey::M,
        Key::N => SerializableKey::N,
        Key::O => SerializableKey::O,
        Key::P => SerializableKey::P,
        Key::Q => SerializableKey::Q,
        Key::R => SerializableKey::R,
        Key::S => SerializableKey::S,
        Key::T => SerializableKey::T,
        Key::U => SerializableKey::U,
        Key::V => SerializableKey::V,
        Key::W => SerializableKey::W,
        Key::X => SerializableKey::X,
        Key::Y => SerializableKey::Y,
        Key::Z => SerializableKey::Z,
        ...
        Key::Plus => SerializableKey::Plus,
        Key::Minus => SerializableKey::Minus,
        Key::Comma => SerializableKey::Comma,
        Key::Period => SerializableKey::Period,
